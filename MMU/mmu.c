...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    node_t * best = NULL;
    block_t * blk;
    int chosen_blocksize = -1;
    int current_blocksize;
    while(current != NULL){
        current_blocksize = current -> blk -> end - current -> blk -> start + 1;
        if (current_blocksize >= blocksize){
            switch(policy){
                ...
                case 2: //BESTFIT
                    if(best == NULL || current_blocksize < chosen_blocksize){
                        best = current;
                        chosen_blocksize = current_blocksize;
                    }
                    break;
                case 3: //WORSTFIT
                    if(best == NULL || current_blocksize > chosen_blocksize){
...
found:
    if(best == NULL){ //no block is found
    ...
    blk = list_remove_at_index(freelist, index); // block is removed from freelist
    if((blk -> end - blk -> start + 1) > blocksize){
        ...
        if(policy == 1){
            ...
            list_add_descending_by_blocksize(freelist, fragment); //handles fragments
        }
        blk -> end = blk -> start + blocksize - 1;
    }
    blk -> pid = pid; //pid is added to the allocated list
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    block_t *blk;
    int index;
    if(!list_is_in_by_pid(alloclist, pid)){ //checks if block is in allocated list
        printf("Error: Can't locate Memory Used by PID: %d\n", pid);
        return;
    }

    index = list_get_index_of_by_Pid(alloclist, pid);
    blk = list_remove_at_index(alloclist, index);//block is removed from allocated list
    blk -> pid = 0; //pid if reset
    if(policy == 1){
